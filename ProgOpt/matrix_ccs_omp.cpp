...
SprMatCCSOMP::~SprMatCCSOMP() {
  this->val.clear();
  this->rows.clear();
  this->ptr.clear();
}

SprMatCCSOMP& SprMatCCSOMP::randMat(int _dim, int spr) {
...
SprMatCCSOMP& SprMatCCSOMP::clrMat() {
  this->cap = 0;
  this->dim = 0;
  this->val.clear();
  this->rows.clear();
...
SprMatCCSOMP SprMatCCSOMP::transMat() {
  SprMatCCSOMP res;
  res.dim = this->dim;
  res.cap = this->cap;
...
SprMatCCSOMP& SprMatCCSOMP::operator=(const SprMatCCSOMP& mat) {
  this->cap = mat.cap;
  this->dim = mat.dim;
  this->val = mat.val;
  this->rows = mat.rows;
...
bool SprMatCCSOMP::operator==(SprMatCCSOMP mat) {
...
SprMatCCSOMP SprMatCCSOMP::operator*(SprMatCCSOMP mat) {
...
SprMatCCSOMP SprMatCCSOMP::ParallelMult(SprMatCCSOMP mat) {
  if (this->dim != mat.dim) throw "wrong sizes";
  SprMatCCSOMP trans = this->transMat();
